...
Pod observability covers monitoring, logging, and interactive debugging of running workloads. These commands help diagnose health issues, view application
output, and inspect cluster resource pressure.

> **Note**: `kubectl top pods` and `kubectl top nodes` require the metrics-server addon to be installed in the cluster. On CCE, install it via
> `hcloud CCE InstallAddon` or the CCE console. If metrics-server is not installed, `top` commands will return `error: Metrics API not available`.

...
- Read-only diagnostic commands are allowed: `hcloud CCE ListClusters`, `ShowCluster`, `ShowClusterEndpoints`, and `kubectl cce ... get`, `describe`, `logs`, `top`, `auth can-i`, `cluster-info`.
...
- This skill must not run `kubectl cce ... apply`, `create`, `patch`, `edit`, `delete`, `scale`, `rollout undo`, `cordon`, `drain`, `taint`, or any equivalent mutating operation.
...
- If scaling, restarting, deleting, rebuilding, node isolation, or quota changes
  are recommended, hand them off to
  `huawei-cloud-cce-auto-remediation-runner` or the relevant domain skill as
  recommendations only.
- Log output must contain only sanitized excerpts. Never copy raw passwords, tokens, AK/SK, kubectl-cce proxy credentials, Authorization headers, or image registry secrets into the output.

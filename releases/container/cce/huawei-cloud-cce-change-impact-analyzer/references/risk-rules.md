...
- Do not roll back, scale, patch, delete, restart, drain, reboot, modify ConfigMap/Secret, modify Service/Ingress/Gateway, modify NetworkPolicy/RBAC, or change cloud network resources.
- Do not use Python SDK dispatcher commands, legacy `huawei-cloud[.]py` entrypoints,
  `skill action=exec`, old change/query actions, kubeconfig generation, direct IAM curl
  flows, or Huawei Cloud SDK imports.
- Use `kubectl cce` for current Kubernetes evidence, hcloud for read-only cloud metadata, and dedicated event/alarm/metric/log skills for historical or observability evidence.
...
- Never retrieve or report Kubernetes Secret values. Collect ConfigMap/Secret metadata only unless the user supplies a sanitized ConfigMap before/after artifact.

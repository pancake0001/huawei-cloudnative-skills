...
def _resolve_project_id(
    ...
    """Return a project ID, resolving it from IAM when the caller omitted it."""
    if project_id:
...
def _get_events_with_cce_plugin(
    ...
    if not resolved_project_id:
        ...
        if not resolved_project_id:
            return {"success": False, "error": f"unable to resolve project_id for kubectl cce: {project_error}"}
    env = os.environ.copy()

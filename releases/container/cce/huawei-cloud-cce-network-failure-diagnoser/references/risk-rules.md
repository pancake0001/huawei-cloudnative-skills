...
- The kubectl-cce plugin blocks `exec`, `attach`, and `port-forward`. Do not bypass that boundary with kubeconfig, SDK, packet capture, active traffic
  generation, or stress tests. Hand active connectivity testing to an approved test path after explicit authorization.

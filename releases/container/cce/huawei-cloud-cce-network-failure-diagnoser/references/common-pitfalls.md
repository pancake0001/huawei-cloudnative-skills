...
For Ingress or LoadBalancer symptoms, first confirm that the Service has ready EndpointSlices. If there are no ready backends, ELB 502/504 is usually
downstream.
...
When Service and endpoints are healthy but east-west traffic fails, inspect NetworkPolicies that select the destination Pod and policy types that imply default
deny.

## Pitfall 4: Overusing Active Tests

Interactive in-Pod HTTP tests, packet capture, and traffic generation can affect workloads or require extra permissions. This skill is passive by default. Use
active tests only after explicit user request and risk acknowledgment.
...
Security groups, ACLs, routes, EIP, and NAT should be cited with actual hcloud read evidence. If the command is not run or permission is missing, mark it as a
verification gap.

...
description: >
  使用 hcloud CLI 获取华为云 CCE 集群信息和短期 kubeconfig，使用 kubectl 做 Kubernetes 预检、路由、压测 Job、日志和指标采集，使用 k6 发起受控流量。用户提到 CCE 压测、负载测试、压力测试、性能测试、k6、ELB 流量测试、全链路压测、弹性评估或流量生成时使用本技能。不要使用 Python SDK dispatcher。
...
```text
hcloud CCE -> 短期 kubeconfig -> kubectl 预检/路由/客户端证据 -> k6 发流 -> kubectl 指标/日志/事件 -> 压测报告
```

集群发现和 kubeconfig 获取使用 CCE hcloud 命令：
...
拿到 kubeconfig 后，Deployment、StatefulSet、DaemonSet、Pod、Service、EndpointSlice、Ingress、HPA、PDB、Event、Job 日志和 metrics-server 指标都用 `kubectl` 读取。
...
5. IAM 至少允许 CCE 集群读取和创建 kubeconfig 证书。只有需要云侧网络证据时才需要 ELB/VPC/EIP/NAT 只读权限。
6. Kubernetes RBAC 允许读取工作负载、Service、EndpointSlice、Ingress、HPA、Event、Pod、Pod 日志、Job 日志和指标。写权限只在用户批准路由、客户端 Job 或扩缩容变更时需要。

不要在报告、命令输出、manifest 或日志中打印 AK、SK、security token、kubeconfig 证书、Authorization header、镜像仓库密钥或应用密钥。
...
确认集群属于预期 region/project，并且当前运行环境能访问 API Server。只有私网端点时，`kubectl` 需要在能访问该私网端点的 VPC/VPN/专线/云桌面环境中运行。

### 3. 获取短期 kubeconfig

使用尽量短的有效期，通常 1 天，并把 kubeconfig 放在仓库外。
...
KooCLI 可能输出 JSON 格式 kubeconfig；`kubectl` 可以直接读取 JSON 或 YAML kubeconfig。刚唤醒集群或刚绑定 EIP 后如果证书创建超时，可增加 CLI 超时和重试参数：
...
2. 用 `hcloud CCE CreateKubernetesClusterCert` 获取 kubeconfig。
3. 用 `kubectl` 做只读预检。
...
`kubectl top` 不可用时，记录为指标缺口，不要编造资源趋势。
...
- `kubectl apply`、`create`、`patch`、`edit`、`delete`、`scale`、`rollout restart`、`rollout undo`
...
- `hcloud CCE ListClusters`、`ShowCluster`、`ShowClusterEndpoints`、`CreateKubernetesClusterCert` 可用。
- `kubectl --kubeconfig=<file>` 能读取目标 namespace 和工作负载。
- 大流量前先执行冒烟压测。

...
The plugin blocks streaming commands such as `exec`, `attach`, and `port-forward`. `logs -f` and `watch` are not hardened. Use bounded `logs --tail` and
ordinary `get`/`describe` commands for reports.

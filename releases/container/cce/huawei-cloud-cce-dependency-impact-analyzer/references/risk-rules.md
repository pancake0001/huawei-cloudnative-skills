...
- Do not use Python SDK dispatcher commands, legacy `huawei-cloud[.]py` entrypoints, `skill action=exec`, old dependency-impact actions, kubeconfig generation,
...
- Static topology alone proves possible impact paths, not real traffic impact. Require logs, metrics, alarms, or user symptoms before claiming actual traffic
  loss.

...
Both public diagnosis tools return structured evidence and a Markdown report. The final customer-facing output should prefer `report_markdown`.

```json
{
  "success": true,
  "action": "huawei_diagnose_cce_hpa_autoscaling | huawei_diagnose_cce_cluster_autoscaler",
...
4. `## 3. Investigation Process`: Gateway and the selected HPA or Cluster Autoscaler execution path.
5. `## 4. Key Evidence`: HPA status, metric APIService, behavior policy, node pool/addon, Pending Pod, FailedScheduling, and other evidence.

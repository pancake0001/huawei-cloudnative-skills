...
- `huawei_generate_cce_hpa_manifest`, `huawei_configure_cce_hpa`: Generate HPA YAML and configuration preview; reusable for remediation but this diagnosis skill
...
- `huawei-cloud-cce-pod-failure-diagnoser`, `huawei-cloud-cce-workload-failure-diagnoser`, `huawei-cloud-cce-node-failure-diagnoser` can handle deep-dive
  escalation.
- `huawei-cloud-cce-cost-optimization-advisor` and `huawei-cloud-cce-capacity-trend-forecaster` already cover HPA coverage, node pool autoscaling,
  request/usage, and capacity trend analysis; their governance recommendations are reusable.
...
- CCE tool family: cluster, node pool, addon, Pod, Deployment, Events, AOM, monitoring, and diagnosis report capabilities already cover most of the base
  evidence needed for autoscaling diagnosis.
...
- Core root cause identification: HPA not configured, missing request, metric missing, threshold not met, maxReplicas, CA not installed/outdated version, node
  pool autoscaling not enabled, max_nodes, no Pending, resource-insufficient Pending, affinity/taint conflict, subnet IP/quota/permission suspicious signals,
  scale-down protection signals.
- **CA Pod log analysis**: Automatically discover CA component Pods in `kube-system` (CCE elastic engine/Cluster Autoscaler), retrieve standard output logs via
  `huawei_get_pod_logs`, match key signals against 16 diagnostic patterns (NoExpansionOptions, MaxNodeGroupSize, QuotaExceeded, SubnetIPExhausted, IAM
  permission, safe-to-evict protection, node not removable, etc.), and merge high-confidence findings into issues and evidence.
...
- `huawei_get_cce_ca_logs`: ~~Read CCE elastic engine/Cluster Autoscaler logs to confirm NoExpansionOptions, MaxNodeGroupSizeReached, QuotaExceeded,
  SubnetIPExhausted, IAM denied reasons.~~ ✅ **Implemented via CA Pod discovery built into `huawei_autoscaling_diagnose` + `huawei_get_pod_logs` combination.**
  The first-release diagnosis already covers 16 key signals from CA logs. A future iteration may encapsulate this as a standalone atomic tool for more flexible
  log queries (e.g., specified time range, LTS integration, debug-level logs).

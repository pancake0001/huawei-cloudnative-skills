...
def get_kubernetes_pods(region: str, cluster_id: str, ak: Optional[str] = None, sk: Optional[str] = None, project_id: Optional[str] = None, namespace: Optional[str] = None, labels: Optional[str] = None, security_token: Optional[str] = None, field_selector: Optional[str] = None) -> Dict[str, Any]:
...
def _workloads(region: str, cluster_id: str, resource: str, key: str, ak: Optional[str], sk: Optional[str], project_id: Optional[str], namespace: Optional[str], security_token: Optional[str] = None, name: Optional[str] = None) -> Dict[str, Any]:
    if error := _namespace_error(namespace):
        return error
    arguments = ["get", resource]
    if name:
        arguments.append(name)
    arguments.extend(["-n", namespace, "-o", "json"])
    result = _kubectl(region, cluster_id, arguments, ak, sk, project_id, security_token)
    if not result.get("success"):
        return result
    rows = []
...
def get_kubernetes_deployments(region: str, cluster_id: str, ak: Optional[str] = None, sk: Optional[str] = None, project_id: Optional[str] = None, namespace: Optional[str] = None, security_token: Optional[str] = None, name: Optional[str] = None) -> Dict[str, Any]:
    return _workloads(region, cluster_id, "deployments", "deployments", ak, sk, project_id, namespace, security_token, name)


def list_cce_statefulsets(region: str, cluster_id: str, namespace: Optional[str] = None, limit: int = 100, include_data: bool = False, ak: Optional[str] = None, sk: Optional[str] = None, project_id: Optional[str] = None, security_token: Optional[str] = None, name: Optional[str] = None) -> Dict[str, Any]:
    result = _workloads(region, cluster_id, "statefulsets", "statefulsets", ak, sk, project_id, namespace, security_token, name)
    if result.get("success"):
        result["statefulsets"] = result["statefulsets"][:limit]
    return result


def get_kubernetes_events(region: str, cluster_id: str, ak: Optional[str] = None, sk: Optional[str] = None, project_id: Optional[str] = None, namespace: Optional[str] = None, limit: int = 200, security_token: Optional[str] = None, event_type: Optional[str] = None, involved_kind: Optional[str] = None, involved_name: Optional[str] = None) -> Dict[str, Any]:
    if error := _namespace_error(namespace):
        return error
    ...
    args.extend(["--chunk-size", str(min(limit, 100))])
    result = _kubectl(region, cluster_id, args, ak, sk, project_id, security_token)
    if not result.get("success"):
        return result
    events = []
    ...
    return {"success": True, "source": "kubectl-cce", "region": region, "cluster_id": cluster_id, "namespace": namespace, "event_type": event_type, "involved_kind": involved_kind, "involved_name": involved_name, "count": len(events), "events": events}


def get_kubernetes_pod_disruption_budgets(region: str, cluster_id: str, ak: Optional[str] = None, sk: Optional[str] = None, project_id: Optional[str] = None, namespace: Optional[str] = None, security_token: Optional[str] = None) -> Dict[str, Any]:
    if error := _namespace_error(namespace):
        return error
    result = _kubectl(region, cluster_id, ["get", "pdb", "-n", namespace, "-o", "json"], ak, sk, project_id, security_token)
    if not result.get("success"):
        return result
    budgets = []
    for item in _items(result):
    ...
    return {"success": True, "source": "kubectl-cce", "region": region, "cluster_id": cluster_id, "namespace": namespace, "count": len(budgets), "pod_disruption_budgets": budgets}

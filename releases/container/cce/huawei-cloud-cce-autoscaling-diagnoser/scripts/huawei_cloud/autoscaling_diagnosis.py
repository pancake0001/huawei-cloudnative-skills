...
CA_ADDON_NAMES = {"autoscaler", "cluster-autoscaler", "cce-elastic-engine", "cce-elastic"}
CA_ADDON_KEYWORDS = ("autoscaler", "cluster-autoscaler", "autoscaling", "弹性引擎", "elastic")
METRIC_ADDON_NAMES = {"metrics-server", "cie-collector", "prometheus"}
METRIC_ADDON_KEYWORDS = ("metrics-server", "cie-collector", "prometheus", "aom", "monitor", "metric")
...
CA_LOG_PATTERNS: list[tuple[re.Pattern, str, str, str]] = [
    ...
    (re.compile(r"(?i)(\\biam\\b|\\bagency\\b|委托|\\bpermission\\s+(?:denied|missing|forbidden)\\b|\\bunauthorized\\b|\\b(?:403|401)\\b)"), "CA_LOG_IAM_PERMISSION_ERROR", "CA 日志显示 IAM/权限异常", "critical"),
...
def _addon_discovery(addons: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for addon in _list(addons.get("addons")):
        names = {_text(addon.get("name")).lower(), _text(addon.get("template_name")).lower()}
        blob = _lower_blob(
            *names,
            addon.get("description"),
            addon.get("version"),
        )
        if names & CA_ADDON_NAMES or any(keyword in blob for keyword in CA_ADDON_KEYWORDS):
            ca_matches.append(addon)
        if names & METRIC_ADDON_NAMES or any(keyword in blob for keyword in METRIC_ADDON_KEYWORDS):
...
def _select_hpas(
    ...
    for hpa in items:
        ...
        if namespace and hpa.get("namespace") != namespace:
            continue
        if hpa_name and hpa.get("name") != hpa_name:
            continue
...
def _events_for_hpa(events: Dict[str, Any], hpa: Dict[str, Any], pods: Iterable[Dict[str, Any]]) -> list[Dict[str, Any]]:
    namespace = hpa.get("namespace")
    name = hpa.get("name")
    target = _target_ref(hpa)
    pod_names = {pod.get("name") for pod in pods}
    selected = []
    for event in _list(events.get("events")):
        ...
        if involved.get("kind") == "HorizontalPodAutoscaler" and involved.get("name") == name:
            ...
            continue
        if involved.get("kind") == target.get("kind") and involved.get("name") == target.get("name"):
            selected.append(event)
            continue
        if involved.get("kind") == "Pod" and involved.get("name") in pod_names:
            selected.append(event)
            continue
...
def _prometheus_series(response: Dict[str, Any]) -> list[Dict[str, Any]]:
    if not response.get("success"):
        return []
    series = _dig(_dict(response.get("result")), "data", "result")
    rows = []
    for item in _list(series):
        values = _list(item.get("values"))
        value = values[-1][1] if values and len(values[-1]) > 1 else _list(item.get("value"))[1] if len(_list(item.get("value"))) > 1 else None
        try:
            numeric_value = float(value)
        except (TypeError, ValueError):
            continue
        rows.append({"labels": _dict(item.get("metric")), "value": numeric_value})
    return rows


def _prometheus_window_summary(response: Dict[str, Any]) -> Dict[str, Any]:
    """Summarize all returned samples instead of treating a range query as a snapshot."""
    if not response.get("success"):
        return {"success": False, "error": response.get("error")}
    values: list[float] = []
    for item in _list(_dig(_dict(response.get("result")), "data", "result")):
        for sample in _list(item.get("values")):
            if len(sample) < 2:
                continue
            try:
                ...
                continue
    if not values:
        return {"success": True, "sample_count": 0}
    return {
        "success": True,
...
def _label_selector(labels: Dict[str, Any]) -> Optional[str]:
    if not labels or any(not isinstance(key, str) or not isinstance(value, str) for key, value in labels.items()):
        return None
...
def _prometheus_label_name(label: str) -> str:
    return "label_" + re.sub(r"[^a-zA-Z0-9_]", "_", label)


def _promql_literal_regex(value: str) -> str:
    """Escape RE2 metacharacters while deliberately preserving ordinary hyphens."""
    return re.sub(r"([\\.^$|?*+()\[\]{}])", r"\\\1", value)


def _hpa_workload(hpa: Dict[str, Any], workloads: Iterable[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    target = _target_ref(hpa)
    return _find_workload(workloads, hpa.get("namespace"), target.get("name"), target.get("kind"))


def _selector_metric_matchers(cluster_id: str, namespace: str, selector: Dict[str, Any]) -> Optional[str]:
    if not selector or any(not isinstance(key, str) or not isinstance(value, str) for key, value in selector.items()):
        return None
    matchers = [f'cluster="{_promql_string(cluster_id)}"', f'namespace="{_promql_string(namespace)}"']
    for key, value in sorted(selector.items()):
        matchers.append(f'{_prometheus_label_name(key)}="{_promql_string(value)}"')
    return ",".join(matchers)


def _collect_hpa_target_resources(
    region: str,
    cluster_id: str,
    hpas: Iterable[Dict[str, Any]],
    ak: Optional[str],
    sk: Optional[str],
    ...
    for hpa in hpas:
        target = _target_ref(hpa)
        namespace, kind, name = hpa.get("namespace"), target.get("kind"), target.get("name")
        if not namespace or not name or kind not in {"Deployment", "StatefulSet"}:
            continue
        target_key = (namespace, kind, name)
        if target_key in seen_targets:
            continue
        seen_targets.add(target_key)

        if kind == "Deployment":
            result = cce.get_kubernetes_deployments(
                region, cluster_id, ak, sk, project_id, namespace, security_token, name=name,
            )
            key, rows = "deployments", deployments
        else:
            result = cce.list_cce_statefulsets(
                region, cluster_id, namespace, 1, False, ak, sk, project_id, security_token, name=name,
            )
            key, rows = "statefulsets", statefulsets
        if not result.get("success"):
            errors.append(f"{namespace}/{kind}/{name}: {result.get('error', 'query failed')}")
            continue
        workload = next(iter(_list(result.get(key))), None)
        if not workload:
            continue
        rows.append(workload)
        selector = _label_selector(_dict(workload.get("selector")))
        if not selector:
            continue
        pod_result = cce.get_kubernetes_pods(
            region, cluster_id, ak, sk, project_id, namespace=namespace, labels=selector,
            security_token=security_token,
        )
        if not pod_result.get("success"):
            errors.append(f"{namespace}/{kind}/{name} Pods: {pod_result.get('error', 'query failed')}")
            continue
        pods.extend(_list(pod_result.get("pods")))

    return {
        "deployments": {"success": not errors, "deployments": deployments, "count": len(deployments), "errors": errors},
        "statefulsets": {"success": not errors, "statefulsets": statefulsets, "count": len(statefulsets), "errors": errors},
        "pods": {"success": not errors, "pods": pods, "count": len(pods), "errors": errors},
    }


def _collect_ca_prometheus_evidence(
    region: str,
    cluster_id: str,
    namespace: str,
    hours: int,
    ...
    instance = cce.get_prom_instance_id(region, cluster_id, ak, sk, project_id, security_token)
    if not instance.get("success"):
        return {"success": False, "error": instance.get("error", "CCE Prometheus instance was not found")}

    aom_instance_id = instance.get("aom_instance_id")
    pending_query = (
        f'max by (namespace, pod) (max_over_time(kube_pod_status_phase{{cluster="{cluster_id}",'
        f'namespace="{namespace}",phase="Pending"}}[5m]))'
    )
    pod_phase_availability_query = (
        f'count(kube_pod_status_phase{{cluster="{cluster_id}",namespace="{namespace}"}})'
    )
    unschedulable_query = (
        f'max_over_time(cluster_autoscaler_unschedulable_pods_count{{cluster="{cluster_id}"}}[5m])'
    )
    pending_result = aom.get_aom_prom_metrics_http(
        region, aom_instance_id, pending_query, hours=hours, ak=ak, sk=sk,
        project_id=project_id, security_token=security_token,
    )
    pod_phase_availability_result = aom.get_aom_prom_metrics_http(
        region, aom_instance_id, pod_phase_availability_query, hours=hours, ak=ak, sk=sk,
        project_id=project_id, security_token=security_token,
    )
    unschedulable_result = aom.get_aom_prom_metrics_http(
        region, aom_instance_id, unschedulable_query, hours=hours, ak=ak, sk=sk,
        project_id=project_id, security_token=security_token,
    )
    if not pending_result.get("success"):
        return {
            ...
            "error": pending_result.get("error", "Pending Pod Prometheus query failed"),
            "promql": {"pending_pods": pending_query, "pod_phase_availability": pod_phase_availability_query, "unschedulable_pods": unschedulable_query},
        }
    pending_pods = [
        {"namespace": item["labels"].get("namespace"), "name": item["labels"].get("pod"), "phase": "Pending", "status": "Pending"}
        for item in _prometheus_series(pending_result)
        if item["value"] > 0 and item["labels"].get("pod")
    ]
    return {
        "success": True,
        "source": "AOM Prometheus",
        "aom_instance_id": aom_instance_id,
        "pending_pods": pending_pods,
        "pending_metric_available": bool(_prometheus_series(pod_phase_availability_result)),
        "unschedulable_pod_metrics": _prometheus_series(unschedulable_result),
        "unschedulable_metric_available": unschedulable_result.get("success", False),
        "promql": {"pending_pods": pending_query, "pod_phase_availability": pod_phase_availability_query, "unschedulable_pods": unschedulable_query},
    }


def _latest_prometheus_value(response: Dict[str, Any]) -> Optional[float]:
    values = _prometheus_series(response)
    return values[0]["value"] if values else None


def _collect_hpa_resource_usage(
    region: str,
    cluster_id: str,
    hpas: Iterable[Dict[str, Any]],
    pods: Dict[str, Any],
    ...
    targets = []
    for hpa in hpas:
        resources = _requested_resource_metrics(hpa)
        workload = _hpa_workload(hpa, workloads)
        selector = _dict(workload.get("selector")) if workload else {}
        target_pods = _pods_for_hpa(hpa, pods)
        if resources and selector:
            targets.append((hpa, resources, selector, target_pods))
    if not targets:
        return {"success": True, "workloads": {}, "skipped": "no HPA target workload selector available"}
    ...
    for hpa, resources, selector, target_pods in targets:
        namespace = _text(hpa.get("namespace"))
        selector_matchers = _selector_metric_matchers(cluster_id, namespace, selector)
        if not selector_matchers:
            continue
        base_labels = f'cluster="{_promql_string(cluster_id)}",namespace="{_promql_string(namespace)}"'
        selector_vector = f'kube_pod_labels{{{selector_matchers}}}'
        measurements = []
        for resource in resources:
            if resource == "cpu":
                base_numerator = f'rate(container_cpu_usage_seconds_total{{{base_labels},image!="",container!="POD"}}[5m])'
            else:
                base_numerator = f'container_memory_working_set_bytes{{{base_labels},image!="",container!="POD"}}'
            numerator = f'sum(({base_numerator}) * on (namespace, pod) group_left() {selector_vector})'
            denominator = f'sum(kube_pod_container_resource_requests{{{base_labels},resource="{resource}"}} * on (namespace, pod) group_left() {selector_vector})'
            query = f'100 * ({numerator} / {denominator})'
            result = aom.get_aom_prom_metrics_http(
                region, instance.get("aom_instance_id"), query, hours=hours, ak=ak, sk=sk,
                project_id=project_id, security_token=security_token,
            )
            summary = _prometheus_window_summary(result)
            source = "workload_selector"
            if result.get("success") and not summary.get("sample_count") and target_pods:
                pod_pattern = "|".join(_promql_literal_regex(_text(pod.get("name"))) for pod in target_pods if pod.get("name"))
                if pod_pattern:
                    pod_labels = f'{base_labels},pod=~"{pod_pattern}"'
                    if resource == "cpu":
                        fallback_numerator = f'sum(rate(container_cpu_usage_seconds_total{{{pod_labels},image!="",container!="POD"}}[5m]))'
                    else:
                        fallback_numerator = f'sum(container_memory_working_set_bytes{{{pod_labels},image!="",container!="POD"}})'
                    fallback_denominator = f'sum(kube_pod_container_resource_requests{{{pod_labels},resource="{resource}"}})'
                    fallback_query = f'100 * ({fallback_numerator} / {fallback_denominator})'
                    result = aom.get_aom_prom_metrics_http(
                        region, instance.get("aom_instance_id"), fallback_query, hours=hours, ak=ak, sk=sk,
                        project_id=project_id, security_token=security_token,
                    )
                    summary = _prometheus_window_summary(result)
                    query = fallback_query
                    source = "target_pod_fallback"
            measurements.append({
                "resource": resource,
                "usage_percent_of_requests": summary.get("latest"),
                "maximum_usage_percent_of_requests": summary.get("maximum"),
                "average_usage_percent_of_requests": summary.get("average"),
        ...
        workloads[f"{hpa.get('namespace')}/{hpa.get('name')}"] = {
            "selector": selector,
            "pod_count": len(target_pods),
            "measurements": measurements,
        }
    return {"success": True, "source": "AOM Prometheus", "aom_instance_id": instance.get("aom_instance_id"), "workloads": workloads}


def _collect_hpa_normal_events(
    region: str,
    cluster_id: str,
    hpas: Iterable[Dict[str, Any]],
    ak: Optional[str],
    ...
    """Fetch only Normal events owned by selected HPAs, notably SuccessfulRescale."""
    events: list[Dict[str, Any]] = []
    errors: list[str] = []
    for hpa in hpas:
        namespace, name = hpa.get("namespace"), hpa.get("name")
        if not namespace or not name:
            continue
        result = cce.get_kubernetes_events(
            region, cluster_id, ak, sk, project_id, namespace=namespace, limit=50,
            security_token=security_token, event_type="Normal",
            involved_kind="HorizontalPodAutoscaler", involved_name=name,
        )
        if not result.get("success"):
            errors.append(f"{namespace}/{name}: {result.get('error', 'query failed')}")
            continue
        events.extend(_list(result.get("events")))
    return {"success": not errors, "events": events, "count": len(events), "errors": errors}


def _pdbs_blocking_scale_down(pods: Iterable[Dict[str, Any]], budgets: Dict[str, Any]) -> list[Dict[str, Any]]:
    blocked = []
    for budget in _list(budgets.get("pod_disruption_budgets")):
        if _as_int(budget.get("disruptions_allowed")) != 0:
            continue
        selector = _dict(budget.get("selector"))
        if not selector:
            continue
        matching = [
            pod for pod in pods
            if pod.get("namespace") == budget.get("namespace")
            and all(_dict(pod.get("labels")).get(key) == value for key, value in selector.items())
        ]
        if matching:
            blocked.append({
                "name": budget.get("name"),
                "namespace": budget.get("namespace"),
                "selector": selector,
...
def _metric_section(metric: Dict[str, Any]) -> tuple[str, Dict[str, Any]]:
    metric_type = _text(metric.get("type") or "Resource")
    return metric_type, _dict(metric.get(metric_type.lower()))


def _metric_identity(metric: Dict[str, Any]) -> str:
    metric_type, section = _metric_section(metric)
    metric_name = section.get("name") or _dict(section.get("metric")).get("name") or "unknown"
    if metric_type == "Object":
        described = _dict(section.get("described_object") or section.get("describedObject"))
        return f"Object/{described.get('kind', 'unknown')}/{described.get('name', 'unknown')}/{metric_name}"
    return f"{metric_type}/{metric_name}"


def _numeric_ratio(current: Any, target: Any) -> Optional[float]:
    try:
        return float(current) / float(target) if target not in {None, 0, "0"} else None
    except (TypeError, ValueError):
        return None


def _metric_entries(hpa: Dict[str, Any]) -> list[Dict[str, Any]]:
    specs = {_metric_identity(metric): metric for metric in _list(hpa.get("metrics"))}
    entries = []
    for metric in _list(hpa.get("current_metrics")):
        ...
        _, spec_section = _metric_section(_dict(specs.get(identity)))
        current = _dict(section.get("current"))
        target = _dict(spec_section.get("target"))
        ...
        current_value = current.get("average_value") or current.get("averageValue") or current.get("value")
        target_value = target.get("average_value") or target.get("averageValue") or target.get("value")
        entries.append({
            "metric": identity,
            ...
            "ratio": _numeric_ratio(current_util if current_util is not None else current_value, target_util if target_util is not None else target_value),
        })
    return entries


def _required_metric_api_services(hpas: Iterable[Dict[str, Any]]) -> set[str]:
    services = set()
    for hpa in hpas:
        for metric in _list(hpa.get("metrics")):
            metric_type = _text(metric.get("type") or "Resource")
            if metric_type == "Resource":
                services.add("v1beta1.metrics.k8s.io")
            elif metric_type in {"Pods", "Object"}:
                services.add("v1beta1.custom.metrics.k8s.io")
            elif metric_type == "External":
                services.add("v1beta1.external.metrics.k8s.io")
    return services


def _collect_metric_api_services(
    region: str,
    cluster_id: str,
    hpas: Iterable[Dict[str, Any]],
    ak: Optional[str],
    ...
    services = []
    for service_name in sorted(_required_metric_api_services(hpas)):
        result = cce._kubectl(
            region,
            cluster_id,
            ["get", "apiservice", service_name, "-o", "json"],
            ak,
            sk,
            project_id,
            security_token,
        ...
        if not result.get("success"):
            services.append({"name": service_name, "available": None, "error": result.get("error")})
            continue
        conditions = _list(_dig(_dict(result.get("data")), "status", "conditions"))
        available = next((item for item in conditions if item.get("type") == "Available"), {})
        services.append({
            "name": service_name,
            "available": _as_bool(available.get("status")),
...
def _hpa_behavior(hpa: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    behavior = _dict(hpa.get("behavior"))
    return {
        "scale_up": _dict(_dig(behavior, "scaleUp")),
        "scale_down": _dict(_dig(behavior, "scaleDown")),
    }
...
def _analyze_hpa_path(
    ...
    pods = raw["pods"]
    events = raw["events"]
    hpa_normal_events = _dict(raw.get("hpa_normal_events"))
    addons = _addon_discovery(raw["addons"])
    metric_api_services = {
        item.get("name"): item
        for item in _list(_dict(raw.get("metric_api_services")).get("services"))
        if item.get("name")
    }
    hpa_resource_usage = _dict(raw.get("hpa_resource_usage")).get("workloads") or {}

    if not selected_hpas:
        scope = f"{namespace or '*'} / {hpa_name or workload_name or '*'}"
        code = "HPA_NOT_FOUND" if hpa_name else "HPA_NOT_CONFIGURED_FOR_WORKLOAD" if workload_name else "HPA_NOT_CONFIGURED"
        _add_issue(
            issues,
            code,
            "未发现匹配的 HPA",
    ...
    for hpa in selected_hpas:
        ...
        if metric_entries:
            current_metric_detail = "; ".join(
                f"{entry.get('metric')}: current="
                f"{entry.get('current_average_utilization') if entry.get('current_average_utilization') is not None else entry.get('current_value')}"
                f", target="
                f"{entry.get('target_average_utilization') if entry.get('target_average_utilization') is not None else entry.get('target_value')}"
                for entry in metric_entries
            )
            evidence.append({
                "layer": "HPA-Metrics",
                "source": "HorizontalPodAutoscaler/status.currentMetrics",
                "summary": current_metric_detail,
            })
        if aggregate_usage:
            usage_detail = "; ".join(
                f"{item.get('resource')} latest={item.get('usage_percent_of_requests'):.2f}%, "
                f"max={item.get('maximum_usage_percent_of_requests'):.2f}%, "
                f"avg={item.get('average_usage_percent_of_requests'):.2f}% "
                f"(samples={item.get('sample_count', 0)}, source={item.get('source')})"
                if item.get("usage_percent_of_requests") is not None else f"{item.get('resource')}=unavailable"
                for item in _list(aggregate_usage.get("measurements"))
            )
            evidence.append({
                "layer": "HPA-Metrics",
                "source": "AOM Prometheus aggregated target Pods",
                "summary": f"target_pods={aggregate_usage.get('pod_count')}; usage/request={usage_detail}",
            })

        for direction, policy in behavior.items():
            if not policy:
                continue
            select_policy = _text(policy.get("select_policy") or policy.get("selectPolicy"))
            stabilization = _as_int(policy.get("stabilization_window_seconds") or policy.get("stabilizationWindowSeconds"))
            evidence.append({
                "layer": "HPA-Behavior",
                "source": "HorizontalPodAutoscaler/spec.behavior",
                "summary": f"{direction}: selectPolicy={select_policy or 'default'}, stabilizationWindowSeconds={stabilization}, policies={len(_list(policy.get('policies')))}",
            })
            if select_policy.lower() == "disabled":
                _add_issue(
                    issues,
                    "HPA_SCALE_DIRECTION_DISABLED",
                    f"HPA {direction} is disabled by behavior policy",
                    "critical" if direction == "scale_up" else "info",
                    "HPA",
                    f"{hpa.get('namespace')}/{hpa.get('name')} spec.behavior.{direction}.selectPolicy=Disabled",
                    f"Review whether disabling {direction} is intentional before changing the HPA behavior policy.",
                )
            elif stabilization > 0:
                _add_issue(
                    issues,
                    "HPA_STABILIZATION_WINDOW_CONFIGURED",
                    f"HPA {direction} has a stabilization window",
                    "info",
                    "HPA",
                    f"spec.behavior.{direction}.stabilizationWindowSeconds={stabilization}",
                    "Consider the configured stabilization window when assessing whether scaling should already have occurred.",
                )

        for service_name in sorted(_required_metric_api_services([hpa])):
            service = metric_api_services.get(service_name)
            if not service:
                _add_issue(
                    issues,
                    "HPA_METRIC_API_NOT_VERIFIED",
                    "HPA metric API service was not verified",
                    "medium",
                    "Metrics",
                    f"No APIService result was collected for {service_name}.",
                    "Verify the required metrics APIService before treating HPA metric availability as healthy.",
                )
                continue
            if service.get("available") is not True:
                _add_issue(
                    issues,
                    "HPA_METRIC_API_UNAVAILABLE",
                    "Required HPA metric API service is unavailable",
                    "critical",
                    "Metrics",
                    f"{service_name}: available={service.get('available')}, reason={service.get('reason') or service.get('error') or ''}",
                    "Restore the required metrics APIService and its backing metrics provider before relying on HPA scaling.",
                )
        ...
        if metric_entries:
            ...
            if below_target and desired_replicas <= current_replicas:
                detail = "; ".join(
                    f"{entry['metric']} current={entry['current_average_utilization'] or entry['current_value']} target={entry['target_average_utilization'] or entry['target_value']}"
            ...
            if in_tolerance and desired_replicas <= current_replicas:
                detail = "; ".join(f"{entry['metric']} ratio={entry['ratio']:.2f}" for entry in in_tolerance)
                _add_issue(
                    issues,
            ...
            if high_entries and desired_replicas <= current_replicas:
                detail = "; ".join(f"{entry['metric']} ratio={entry['ratio']:.2f}" for entry in high_entries)
                _add_issue(
                    issues,
...
def _fetch_ca_pod_logs(
    ...
    kube_pods = cce.get_kubernetes_pods(region, cluster_id, ak, sk, project_id, namespace="kube-system", security_token=security_token)
...
def _analyze_ca_path(
    ...
    prometheus = raw.get("ca_prometheus", {})
    # Node autoscaling must evaluate every Pending Pod in the requested namespace.
    # An unrelated HPA must not narrow this scope and hide a scale-up trigger.
    pending = _list(prometheus.get("pending_pods")) if prometheus.get("pending_metric_available") else _pending_pods_for_scope(raw, namespace, [])
    ...
    if prometheus.get("success"):
        evidence.append({
            "layer": "CA-Metrics",
            "source": "AOM Prometheus",
            "summary": (
                f"pending_metric_available={prometheus.get('pending_metric_available')}, pending_pods={len(pending)}, "
                f"unschedulable_metric_available={prometheus.get('unschedulable_metric_available')}"
            ),
        })
    ...
    if direction in {"scale_down", "unknown"}:
        ...
        safe_to_evict = [
            pod for pod in pods
            if _text(_dict(pod.get("annotations")).get(SAFE_TO_EVICT_KEY)).lower() == "false"
        ]
        kube_system_non_ds = [
            pod for pod in pods
        ...
        pdb_blocks = _pdbs_blocking_scale_down(pods, raw.get("pod_disruption_budgets", {}))
        if safe_to_evict:
            ...
            _add_issue(
                ...
                "CA",
                sample,
                "将 safe-to-evict 改为 true 或移除该注解前，先确认 Pod 可以安全重建。",
            )
        if pdb_blocks:
            sample = "; ".join(
                f"{item['namespace']}/{item['name']} protects {len(item['matching_pods'])} matching Pod(s)"
                for item in pdb_blocks[:5]
            )
            _add_issue(
                issues,
                "SCALE_DOWN_PDB_BLOCKING",
                "PodDisruptionBudget currently blocks eviction",
                "high",
                "CA",
                sample,
                "Review the PDB budget and workload replica availability before allowing disruption.",
            )
...
def collect_autoscaling_context(
    ...
    selected_hpas = _select_hpas(hpas, namespace, hpa_name, workload_name, workload_type)
    is_node_scale_up = target == "NODE" and scale_direction == "scale_up"
    requires_scale_down_data = target == "NODE" and scale_direction in {"scale_down", "unknown"}
    hpa_targeted_collection = target == "WORKLOAD" and bool(hpa_name or workload_name)
    hpa_resources = (
        _collect_hpa_target_resources(region, cluster_id, selected_hpas, ak, sk, project_id, security_token)
        if hpa_targeted_collection else None
    )
    ca_prometheus = (
        _collect_ca_prometheus_evidence(region, cluster_id, namespace or "", hours, ak, sk, project_id, security_token)
        if is_node_scale_up and include_metrics else {"success": False, "skipped": "not required for this diagnostic path"}
    )
    raw: Dict[str, Dict[str, Any]] = {
        "hpas": hpas,
        "addons": cce.list_cce_addons(region, cluster_id, ak, sk, project_id, security_token),
        "nodepools": cce.list_cce_node_pools(region, cluster_id, ak, sk, project_id, limit=100, security_token=security_token),
        "ca_prometheus": ca_prometheus,
        "pods": (
            {"success": True, "pods": [], "count": 0, "skipped": "AOM Prometheus Pending Pod metric available"}
            if is_node_scale_up and ca_prometheus.get("pending_metric_available") else cce.get_kubernetes_pods(
                region, cluster_id, ak, sk, project_id, namespace=namespace,
                field_selector="status.phase=Pending" if is_node_scale_up else None,
                security_token=security_token,
            )
        ),
        "events": cce.get_kubernetes_events(
            region, cluster_id, ak, sk, project_id, namespace=namespace,
            limit=event_limit, event_type="Warning", security_token=security_token,
        ),
        "deployments": (
            {"success": True, "deployments": [], "count": 0, "skipped": "not required for node scale-up"}
            if is_node_scale_up else hpa_resources["deployments"] if hpa_resources else cce.get_kubernetes_deployments(region, cluster_id, ak, sk, project_id, namespace=namespace, security_token=security_token)
        ),
        "statefulsets": (
            {"success": True, "statefulsets": [], "count": 0, "skipped": "not required for node scale-up"}
            if is_node_scale_up else hpa_resources["statefulsets"] if hpa_resources else cce.list_cce_statefulsets(region, cluster_id, namespace, 100, False, ak, sk, project_id, security_token)
        ),
        "pod_disruption_budgets": (
            cce.get_kubernetes_pod_disruption_budgets(region, cluster_id, ak, sk, project_id, namespace, security_token)
            if requires_scale_down_data else {"success": True, "pod_disruption_budgets": [], "count": 0, "skipped": "not required for scale-up"}
        ),
    }
    if hpa_resources:
        raw["pods"] = hpa_resources["pods"]
    if hpas.get("success"):
        raw["metric_api_services"] = _collect_metric_api_services(
            region,
            cluster_id,
            _list(hpas.get("hpas")),
            ak,
    ...
    if target == "WORKLOAD" and selected_hpas:
        raw["hpa_normal_events"] = _collect_hpa_normal_events(
            region, cluster_id, selected_hpas, ak, sk, project_id, security_token,
        )
    if include_metrics and target == "WORKLOAD":
        raw["hpa_resource_usage"] = _collect_hpa_resource_usage(
            region, cluster_id, selected_hpas, raw["pods"],
            _workload_rows(raw["deployments"], raw["statefulsets"]),
            hours, ak, sk, project_id, security_token,
        )
    if include_ca_logs:
        raw["ca_pod_logs"] = _fetch_ca_pod_logs(region, cluster_id, ak, sk, project_id, security_token, tail_lines=ca_log_tail_lines)
    return raw
...
def assess_autoscaling_context(
    ...
    if route == "BLOCKED":
        ...
        if route in {"A", "C"}:
            ...
            hpa_result = _analyze_hpa_path(raw, selected_hpas, workloads, namespace, hpa_name, workload_name, workload_type, tolerance)
    ...
    result: Dict[str, Any] = {
        "success": True,
        "action": "huawei_diagnose_cce_hpa_autoscaling",
        "generated_at": _now(),
        "region": region,
        "cluster_id": cluster_id,
        "intent": {"target": target_intent, "scale_direction": direction, "question": question},
        "scope": {"namespace": namespace, "hpa_name": hpa_name, "workload_name": workload_name, "workload_type": workload_type},
        "route": route,
        "discovery": discovery,
        "process": process,
...
def diagnose_cce_autoscaling(
    region: str,
    cluster_id: str,
    action: str = "huawei_diagnose_cce_hpa_autoscaling",
    question: str = "",
    target: Optional[str] = None,
    scale_direction: Optional[str] = None,
    namespace: Optional[str] = None,
    ...
    raw = collect_autoscaling_context(
        ...
        target=classify_intent(question, target),
        scale_direction=classify_scale_direction(question, scale_direction),
        hpa_name=hpa_name,

...
- Do not automatically patch the internal `bursting-status` ConfigMap. Treat it as read-only diagnostic evidence unless the installed addon version documents a
...
- Prefer a tenant-owned regional SWR image for the smoke workload. Public namespace images are fallback-only because CCI image pulling through VPCEP may fail
  for them.

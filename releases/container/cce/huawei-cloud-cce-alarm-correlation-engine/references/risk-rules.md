...
- Cloud resource changes are allowed only through the tools provided by this skill; do not modify alarm rules, action rules, mute rules, or other cloud
  resources through any out-of-band method.
- Output must never expose AK/SK, tokens, or complete sensitive logs.
- If the user requests scaling, rebooting, draining, or other remediation actions, output recommendations only and hand off to

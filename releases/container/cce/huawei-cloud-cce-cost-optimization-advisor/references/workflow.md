...
2. Collect namespace allocation and usage for every namespace by default. System namespaces are shown for visibility but are not eligible for rightsizing or cost-reduction recommendations; honor any explicit exclusions from the user.
...
Show every namespace in the resource-cost view, but perform oversized-request and rightsizing assessment only for non-system workloads.

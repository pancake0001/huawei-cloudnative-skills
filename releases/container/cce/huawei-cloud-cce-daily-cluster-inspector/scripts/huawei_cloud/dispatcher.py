...
def _normalize_cli_credentials(params: Dict[str, str]) -> tuple[Dict[str, str], str | None]:
    """Use explicitly supplied CLI credentials without silently mixing sources."""
    normalized = dict(params)
    for cli_key, internal_key in (
        ("cli_access_key", "ak"),
        ("cli_secret_key", "sk"),
        ("cli_security_token", "security_token"),
    ):
    ...
    if bool(normalized.get("ak")) != bool(normalized.get("sk")):
        return params, "cli_access_key and cli_secret_key must be provided together"
    if normalized.get("security_token") and not normalized.get("ak"):
...
_SKILL_NAME = Path(__file__).resolve().parents[2].name
_PUBLIC_ACTIONS = {
    "huawei-cloud-cce-autoscaling-diagnoser": {
        "huawei_autoscaling_diagnose", "huawei_list_cce_clusters", "huawei_list_cce_hpas",
        "huawei_list_cce_addons", "huawei_get_cce_addon_detail", "huawei_list_cce_nodepools",
        "huawei_get_kubernetes_nodes", "huawei_get_cce_pods", "huawei_get_cce_deployments",
        "huawei_list_cce_statefulsets", "huawei_get_cce_events", "huawei_get_cce_pod_metrics_topN",
        "huawei_get_cce_node_metrics_topN", "huawei_get_aom_metrics",
    },
    "huawei-cloud-cce-availability-risk-scanner": {
        "huawei_scan_cce_availability_risk", "huawei_list_cce_clusters", "huawei_get_kubernetes_nodes",
        "huawei_get_cce_pods", "huawei_get_cce_deployments", "huawei_get_cce_services",
        "huawei_get_cce_ingresses", "huawei_list_cce_nodepools", "huawei_list_cce_daemonsets",
        "huawei_list_cce_statefulsets", "huawei_get_cce_node_metrics_topN", "huawei_get_aom_metrics",
    },
    "huawei-cloud-cce-capacity-trend-forecaster": {
        "huawei_analyze_cce_capacity_trend", "huawei_list_cce_clusters", "huawei_get_kubernetes_nodes",
        "huawei_list_cce_nodepools", "huawei_get_cce_deployments", "huawei_list_cce_hpas",
        "huawei_get_cce_node_metrics_topN", "huawei_get_aom_metrics", "huawei_generate_cce_hpa_manifest",
    },
    "huawei-cloud-cce-cost-optimization-advisor": {
        "huawei_list_cce_clusters", "huawei_analyze_cce_cost_optimization", "huawei_list_cce_nodes",
        "huawei_get_kubernetes_nodes", "huawei_list_cce_nodepools", "huawei_get_cce_pods",
        "huawei_get_cce_deployments", "huawei_list_cce_hpas", "huawei_generate_cce_hpa_manifest",
        "huawei_get_cce_node_metrics_topN", "huawei_get_cce_node_metrics", "huawei_get_cce_pod_metrics_topN",
        "huawei_get_cce_pod_metrics", "huawei_get_aom_metrics", "huawei_generate_monitor_dashboard",
    },
    "huawei-cloud-cce-daily-cluster-inspector": {
        "huawei_cce_quick_check", "huawei_cce_auto_inspection", "huawei_cce_deep_diagnosis",
        "huawei_cce_cluster_inspection", "huawei_cce_cluster_inspection_parallel",
        "huawei_cce_cluster_inspection_subagent", "huawei_aggregate_inspection_results",
        "huawei_export_inspection_report", "huawei_pod_status_inspection", "huawei_node_status_inspection",
        "huawei_node_resource_inspection", "huawei_event_inspection", "huawei_aom_alarm_inspection",
        "huawei_elb_monitoring_inspection",
    },
}
ACTION_SPECS = {action: spec for action, spec in ACTION_SPECS.items() if action in _PUBLIC_ACTIONS[_SKILL_NAME]}
...
def dispatch_action(action: str, params: Dict[str, str]) -> Dict[str, Any]:
    ...
    input_cluster_id = params.get("cluster_id")
    if input_cluster_id:
        resolution = common.resolve_cce_cluster_id(
            params["region"], input_cluster_id, params.get("ak"), params.get("sk"),
            params.get("project_id"), params.get("security_token"),
        )
        if not resolution.get("success"):
            return resolution
        params["cluster_id"] = resolution["id"]
    result = handler(params)
    if input_cluster_id and result.get("success") and input_cluster_id != params["cluster_id"]:
        result.setdefault("resolved_resource_ids", []).append({"parameter": "cluster_id", "input": input_cluster_id, "resolved_id": params["cluster_id"]})
    return result

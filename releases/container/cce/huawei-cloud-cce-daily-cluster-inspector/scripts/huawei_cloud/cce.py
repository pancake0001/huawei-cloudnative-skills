...
def _kubectl(region: str, cluster_id: str, arguments: List[str], ak: Optional[str], sk: Optional[str], project_id: Optional[str], security_token: Optional[str] = None, expect_json: bool = True) -> Dict[str, Any]:
    access_key, secret_key, resolved_project_id = common.get_credentials_with_region(
        region,
        ak,
        sk,
        project_id,
...
def show_cce_cluster(region: str, cluster_id: str, ak: Optional[str] = None, sk: Optional[str] = None, project_id: Optional[str] = None, security_token: Optional[str] = None) -> Dict[str, Any]:
    result = _hcloud(region, "ShowCluster", ak, sk, project_id, security_token, cluster_id=cluster_id)
    if not result.get("success"):
        return result
    data = result.get("data") or {}
    metadata, spec, status = data.get("metadata") or {}, data.get("spec") or {}, data.get("status") or {}
    return {"success": True, "source": "hcloud", "region": region, "cluster": {"id": metadata.get("uid", cluster_id), "name": metadata.get("name"), "status": status.get("phase") or status.get("status"), "type": spec.get("type"), "version": spec.get("version")}}
...
def get_kubernetes_events(region: str, cluster_id: str, ak: Optional[str] = None, sk: Optional[str] = None, project_id: Optional[str] = None, namespace: Optional[str] = None, limit: int = 500, security_token: Optional[str] = None, warning_only: bool = False) -> Dict[str, Any]:
    if error := _namespace_error(namespace):
        return error
    args = ["get", "events", "-n", namespace]
    if warning_only:
        args.append("--field-selector=type=Warning")
    args.extend(["--chunk-size=200", "--request-timeout=45s", "-o", "json"])
    result = _kubectl(region, cluster_id, args, ak, sk, project_id, security_token)
    if not result.get("success"):
        return result
    events = []
...
def get_kubernetes_nodes(region: str, cluster_id: str, ak: Optional[str] = None, sk: Optional[str] = None, project_id: Optional[str] = None, node_name: Optional[str] = None, security_token: Optional[str] = None) -> Dict[str, Any]:
    args = ["get", "node"] + ([node_name] if node_name else ["--chunk-size=200"]) + ["--request-timeout=45s", "-o", "json"]
    result = _kubectl(region, cluster_id, args, ak, sk, project_id, security_token)
    if not result.get("success"):
        return result
    raw = result.get("data") or {}
    items = raw.get("items") or ([raw] if raw.get("metadata") else [])
    nodes = [{"name": (item.get("metadata") or {}).get("name"), "labels": (item.get("metadata") or {}).get("labels") or {}, "conditions": (item.get("status") or {}).get("conditions") or [], "taints": (item.get("spec") or {}).get("taints") or [], "unschedulable": bool((item.get("spec") or {}).get("unschedulable"))} for item in items]

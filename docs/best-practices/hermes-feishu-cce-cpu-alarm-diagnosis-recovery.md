...
6. 对涉及资源变更的动作，只生成恢复预览，不直接执行任何现网动作。
7. 只有用户在飞书中明确回复“确认执行”或确认具体方案后，才执行恢复动作。
...
- 在用户确认前，禁止执行扩缩容、回滚、重启、节点池变更等任何会改变现网状态的动作。
...
- 需要恢复时，明确提示用户回复“确认执行”或选择具体方案；在收到人工确认前，只等待确认，不执行变更。
...
这是生产变更链路中的关键审核点。Agent 在该阶段只等待用户确认，不会执行扩缩容、回滚、重启、节点池变更等任何会改变现网状态的动作。只有当用户在飞书中明确回复“确认执行”或“确认执行方案 A/B”等确认语句后，Agent 才会按已确认的方案执行恢复动作。

确认语句应与恢复预览中的方案编号保持一致，例如“确认执行方案 A”对应扩容副本，“确认执行方案 B”对应资源升配或容量扩展。客户也可以将确认动作替换为飞书卡片按钮、工单审批或企业审批流，但核心要求不变：未经过人工确认，Agent 只做分析和预览，不做现网变更。

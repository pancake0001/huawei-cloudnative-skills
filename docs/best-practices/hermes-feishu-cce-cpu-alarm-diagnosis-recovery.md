...
客户可以结合自身环境逐步准备以下能力，不要求一次性完成全部自动化：

- 准备用于巡检和诊断的 CCE 集群、命名空间或业务范围。
- 接入 AOM 告警、云原生监控指标或已有巡检对象。
- 准备 Hermes 或客户自有 Agent Runtime，并接入飞书、工单或其他值班通道。
- 按需注册华为云云原生 Skill，用于查询告警、指标、事件、工作负载和节点状态。
- 对恢复类动作保留“预览 + 用户确认”机制。
- 通过安全方式提供访问凭证，不在提示词、文档、截图或飞书消息中暴露敏感信息。

## Hermes 任务提示词参考

以下提示词可作为 Hermes ChatOps 值班 Agent 的任务模板。客户如果使用其他 Agent，也可以参考其中的角色定义、处理流程和安全边界，自定义为 Pod 诊断助手、节点运维助手、容量巡检助手、发布变更分析助手等不同类型的智能运维助手。
...
2. 对告警进行去重、归并、分级和影响范围摘要。
3. 对需要处理的告警自动汇聚上下文，包括 AOM 告警、指标、Kubernetes Events、Pod/Workload/Node 状态、日志摘要和近期变更。
4. 输出面向值班人员的诊断报告，包含告警摘要、影响对象、关键证据、候选原因、建议方案和需要确认的动作。
5. 对涉及资源变更的动作，只生成恢复预览，不直接执行。
6. 只有用户在飞书中明确确认后，才执行恢复动作。
7. 执行后必须验证告警状态、Pod 状态、工作负载副本、节点容量和关键指标，并将闭环结果发送到飞书。

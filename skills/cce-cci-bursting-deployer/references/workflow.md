...
`huawei_precheck_cce_cci_bursting` includes NodeCheck automatically. For a small validation cluster, the action warns when it cannot find a schedulable physical node with at least 2 vCPU and 4 GiB of currently unrequested capacity, or when fewer than two schedulable physical nodes are available.

The 2C/4GiB value is a conservative warning baseline, not a platform hard limit. Size production addon resources with the official formula and the expected number of synchronized resources and concurrent requests.
...
The setup action resolves the regional Huawei Cloud project ID from the environment or IAM API and writes it to the addon values. Pass `project_id=<regional-project-id>` explicitly when IAM auto-resolution is unavailable.
